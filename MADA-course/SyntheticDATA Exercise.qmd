...
ggplot(data, aes(x = Dose, y = Cholesterol_Level, color = Treatment_Group)) +
  geom_point() +
...
ggplot(data, aes(x = Dose, y = CRP_Level, color = Treatment_Group)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
ggplot(data, aes(x = Time, y = 1 - Heart_Attack, color = Treatment_Group)) +
  geom_step() +
...
model_cholesterol <- lm(Cholesterol_Level ~ Dose, data = data)

# Fit simple linear regression model for drug dosage and CRP level
model_crp <- lm(CRP_Level ~ Dose, data = data)

# Fit simple logistic regression model for drug dosage and heart attack event
model_heart_attack <- glm(Heart_Attack ~ Dose, data = data, family = "binomial")
